...
#' @export
#' @usage TargetScans_data(MAE, targetScan, species = '')
#' @examples
#'library(tidyverse)
...
#'TargetScans_df <- TargetScans_data(MAE, targetScan = assay(MAE, 1),
#'                                   species = 'mmu')
TargetScans_data <- function(MAE, targetScan, species){
    ...
    TargetScans_s <- targetScan %>% filter(str_detect(Transcript, species))
    if (species == 'hsa') {
