...
#' pathway of interest and miRNA data.
...
#' @return A matrix of information which cross references the occurances of
...
dataMiningMatrix <- function(MAE, corrTable, targetscan, mirdb,
    ...
    if (missing(corrTable)) stop('Input matrix of miR-mRNA interactions and
                                 correlations. This is output from the
                                 mirMrnaInt function.')
    ...
    df3 <- df2 %>% dplyr::mutate(miRDB = as.integer(
                                 rownames(df2) %in% mirdb[[1]]))

    rownames(df3) <- rownames(corrTable)

    # Which occur in both?
    df4 <- df3 %>% dplyr::mutate(Predicted_Interactions = TargetScan + miRDB)

    rownames(df4) <- rownames(corrTable)

    # Are miR-mRNA interactions found in mirtarbase?
    df5 <- df4 %>% dplyr::mutate(miRTarBase = as.integer(
                                 rownames(df4) %in% mirtarbase[[1]]))

    rownames(df5) <- rownames(corrTable)

    # Which occur in all 3?
    df6 <- df5 %>% dplyr::mutate(Pred_Fun = Predicted_Interactions + miRTarBase)
