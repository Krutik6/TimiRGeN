...
#' or average expression, as this data will be averaged and correlated.
#' This is to be used for miR and mRNA data individually. The results will be
#' stored as assays in a MAE object.
...
#' identified by enrichWiki or returnCluster function. It is recommended to
...
#' columns.) These will be found as assays in the MAE object used in the
...
#'  data.
#' @param genes_ID Dataframe with that was created from a getIds
#' function e.g. mRNA_ensembl or miR_entrez. Use the same ID type for miR
#' and mRNA data. These dataframes will be found as assays within the MAE
#' which stores results from the getIds functions.
#' @param idColumn Name of column to use as the merge point. If Column names in
#' getIds results have not been changed, it should be 'GENENAME'.
#' @param name = New name of the dataframe. Should be a unique string.
#' @return Returns only a single result type from DE e.g. Log2FC and an ID type
#' e.g. entrezIDs.
#' @export
...
diffExpressRes <- function(MAE, df, dataType, genes_ID, idColumn, name){
    ...
    if (missing(MAE)) stop('Add a MAE object. This will store output from
                           diffExpressRes. Please use the startObject and
                           getIds functions first.')

    if (missing(df)) stop('Add miR or mRNA dataframe which contains genes
    ...
    if (missing(dataType)) stop('Add common name for expression columns from
                                 data. e.g "AveExp", "Log2fc", "adjP.val".')

    if (missing(genes_ID)) stop('Add dataframe from getIDs functions.
    ...
    if (missing(idColumn)) stop('Add which column from the getIds results
                                name to use for merge point. e.g. "GENENAME."
                                If column names have not been renamed, then
                                leave as "GENENAME"')

    if (missing(name)) stop('Add name of new data frame. This should be
                            a unique string. A single MAE object cannot store
