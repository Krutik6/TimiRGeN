...
#' @param path_gene path_gene dataframe. From downladGMT or gmtEnsembl.
#' @param path_name path_name dataframe from downladGMT.
...
enrichWiki <- function(MAE, method, ID_list, orgDB, path_gene, path_name, ID,
    ...
    if (missing(path_gene)) stop ('Use downloadGMT/ gmtEnsembl to get
                                  wikipathway data.')
    ...
    if (missing(universe)) stop ('Add a list of gene IDs to be used as the
                                  ...
                                  The reccomended method is to use
    ...
    # Extract information in an orgaised way
