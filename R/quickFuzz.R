...
#' @param Clusters Output after createClusters. A list of clusters.
...
quickFuzz <- function(Mfuzzdata, Clusters, W = TRUE){
    ...
    if (missing(Mfuzzdata)) stop('Add Mfuzzdata from the ExpressionList of an
                                 MAE object, after createClusters.')

    if (missing(Clusters)) stop('Add Clusters from the metadata of an
                                 MAE object, after createClusters.')
