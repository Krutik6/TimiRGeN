...
readRDS("wpdata.rds") -> MAE
MAE[[1]] -> path_gene
MAE[[3]] -> path_data
#test GMT_ensembl function
GMT_ensembl(MAE = MAE, path_gene = path_gene,
path_data = path_data, orgDB = org.Mm.eg.db) -> MAE
#internal checks
...
expect_equal(names(path_data_ensembl_corrected),
names(MAE@ExperimentList$path_data_ensembl))
expect_equal(names(path_gene_ensembl_manual), 
names(MAE@ExperimentList$path_gene_ensembl))
...
expect_equal(path_gene_ensembl_manual, MAE@ExperimentList$path_gene_ensembl)
expect_equal(path_data_ensembl_corrected, MAE@ExperimentList$path_data_ensembl)
})
