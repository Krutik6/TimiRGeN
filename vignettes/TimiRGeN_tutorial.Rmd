...
databases curated within the last 4 years. TimiRGeN also only uses databases
...
libstoinst <- c("TimiRGeN", "gtools", "rWikiPathways", "tidyr","stringr",
                 "tidyverse", "clusterProfiler", "biomaRt", "org.Hs.eg.db",
                "org.Mm.eg.db", "RCy3", "ggplot2", "dplyr", "igraph",
                "Mfuzz", "readxl", "MultiAssayExperiment")
...
Currently the package can analyse human and mouse data, can analyse miR
and mRNA data combined or separately, and use entrez or ensembl gene IDs.
...
Note. There should only be one '.'  in each column name and no '-' characters.
Having more than one '.' any '-' characters will confuse some functions.
...
MAE <- getIDs_miR_mouse(MAE = MAE, assay(MAE, 1))
MAE <- getIDs_mRNA_mouse(MAE = MAE, assay(MAE, 2))
```
...
```{r CreateClusters_slow, eval=FALSE, fig.align ="center",dpi=80}
MAE2 <- CreateClusters(MAE = MAE2, method = "c",
                       percentMatrix = assay(MAE2, 5),
                       noClusters = 12, variance = 0.99)
```

```{r CreateClusters_working, echo=FALSE, fig.align ="center",dpi=80}
C <- Clusters
MAE2 <- c(MAE2, C)
```
...
MAE2 <- Express(MAE = MAE2, df = assay(MAE, 1), dataType = "Log2FC",
                genes_ID = assay(MAE, 3), idColumn = "GENENAME",
                name = "miR_log2fc")

MAE2 <- Express(MAE = MAE2, df = assay(MAE, 2), dataType = "Log2FC",
                genes_ID = assay(MAE, 7), idColumn = "GENENAME",
                name = "mRNA_log2fc")
```

The `Express` function will retrieve one specific result type from differential
...
instead the mouse or human interactions can be loaded from `TimiRGeN` using
the code below.

For human analysis use `"hsa"` and `org.Hs.eg.db`.

```{r Targetscans data, message=FALSE, warning=FALSE, eval=FALSE}
MAE3 <- dloadTargetScans(MAE = MAE3)
MAE3 <- TargetScans_data(MAE = MAE3,
                         targetScan = assay(MAE3, 4),
                         species = "mmu")

MAE3 <- dloadmiRDB(MAE = MAE3)
MAE3 <- miRDB_data(MAE = MAE3,
                   miRDB = assay(MAE3, 2),
                   species = "mmu",
                   orgdb = org.Mm.eg.db)
...
MAE3 <- MatrixFilter(MAE = MAE3, miningMatrix = assay(MAE3, 7),
                     negativeOnly = TRUE,
                     predictedOnly = FALSE,
                     threshold = 2,
                     maxCor = 0.5)
...
MAE3 <- MakeNet(MAE = MAE3, filt_df = assay(MAE3, 8))
...
write.table(assay(MAE3, 9), "MAPP.txt", quote = FALSE,
row.names = FALSE, col.names = FALSE, sep = "\t")

write.csv(assay(MAE3, 10), "Dynamics.csv", row.names = TRUE,
quote = FALSE)
```


TimiRGeN creates two types of files which can be imported by
pathvisio [12]. Firstly, Dynamic data which can be imported as a data set to
...
change the nomenclature to run it through TimiRGeN.
...
MAE <- getIDs_miR_human(MAE = MAE, assay(MAE, 1))
MAE <- getIDs_mRNA_human(MAE = MAE, assay(MAE, 2))
```
...
MAE2 <- GMT_ensembl(MAE = MAE2, path_gene = assay(MAE2, 1),
...
Data <- getIDs_mRNA_mouse(MAE = Data, mRNA = assay(Data, 2),
                          mirror = 'useast')
...
```{r eval=FALSE}
Quickbar(X = metadaya(Data2)[[1]][[1]], Y = metadaya(Data2)[[1]][1])
```
